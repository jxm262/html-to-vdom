...
module.exports = function createConverter (VNode, VText) {
    var converter = {
        ...
        convertTag: function (tag, getVNodeKey) {
            ...
            var tagChildrenLength = tagChildren.length
            var children = new Array(tagChildren.length);
            var convertFunction = converter.convert;
            for (var i = 0; i < tagChildrenLength; i++) {
                children[i] = convertFunction(tagChildren[i], getVNodeKey);
            }
